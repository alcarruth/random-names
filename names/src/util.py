...
def mk_names_src_py():
    names_src_py = open('names_src.py', 'w')

    for name_type in ['female_names', 'male_names', 'surnames']:
        f = open(name_type + '.json')
        names = json.loads(f.read())
        f.close()
        names_src_py.write(name_type + ' = [\n')
        for name in names:
            names_src_py.write('    ' + name + ',\n')
        names_src_py.write(']\n\n')
...
def name_generator(gender=['male','female'], n=20):
    given_names = []
    for g in gender:
        given_names += names['given'][g]
